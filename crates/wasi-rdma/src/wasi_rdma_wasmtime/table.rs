...
impl Table {
    ...
    pub fn push<T: Any + Send + Sync>(&self, a: Arc<T>) -> Result<u32, Error> {
        ...
        if inner.map.len() == u32::MAX as usize {
            return Err(anyhow::Error::msg("table has no free keys"));
        }
        loop {
    ...
    pub fn get<T: Any + Send + Sync + Sized>(&self, key: u32) -> Result<Arc<T>, Error> {
        if let Some(r) = self.0.read().unwrap().map.get(&key).cloned() {
            r.downcast::<T>()
                .map_err(|_| anyhow!("element is a different type"))
        } else {
            Err(anyhow!("key not in table"))
        }
    }
    ...
    pub fn get_mut<T: Any>(&mut self, key: u32) -> Result<&mut T, Error> {
        let entry = match self.0.get_mut().unwrap().map.get_mut(&key) {
            Some(entry) => entry,
            None => return Err(anyhow!("key not in table")),
        };
        let entry = match Arc::get_mut(entry) {
            Some(entry) => entry,
            None => return Err(anyhow!("cannot mutably borrow shared file")),
        };
        entry
            .downcast_mut::<T>()
            .ok_or_else(|| anyhow!("element is a different type"))
    }
    ...
    pub fn renumber(&self, from: u32, to: u32) -> Result<(), Error> {
        ...
        let from_entry = map.remove(&from).ok_or(anyhow!("remove error!"))?;
